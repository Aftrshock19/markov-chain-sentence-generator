...
from surface_features import infer_surface_features as infer_verb_surface_features
from route_planner import RoutePlan, build_route_plan as build_verb_route_plan
from verb_routes import VERB_ROUTE_FAMILY_BONUS, VERB_ROUTE_TEMPLATE_PREFIX, route_family_from_template_id, template_id_for_route
...
class SentenceGenerator:
    ...
    def strategy_validation_reasons(self, candidate: Candidate, words: List[str], finite_verb_index: int) -> List[str]:
        ...
        route_plan = self.build_route_plan(self.lexicon.get(candidate.lemma)) if candidate.pos == "v" else None
        route_reasons = self.verb_route_structural_reasons(candidate, route_plan)
        if route_reasons:
    ...
    def strategy_score_adjustment(self, candidate: Candidate) -> float:
        family = self.normalized_pos_family(self.lexicon.get(candidate.lemma))
        adjustment = 0.0
        route_family = route_family_from_template_id(candidate.template_id)
        if self.exact_surface_required(candidate):
            adjustment += 0.4
        if route_family:
            adjustment += VERB_ROUTE_FAMILY_BONUS.get(route_family, 0.0)
            if len(self.word_tokens(candidate.sentence)) <= 4:
                adjustment += 0.15
    ...
    def infer_surface_features(self, entry: Lexeme) -> Dict[str, Any]:
        if not entry or self.normalized_pos_family(entry) != "v":
            return {}
        return infer_verb_surface_features(
            entry,
            self.canonical_lemma_for(entry),
            self.target_form_metadata(entry),
        )

    def build_route_plan(self, entry: Lexeme) -> Optional[RoutePlan]:
        if not entry or self.normalized_pos_family(entry) != "v":
            return None
        return build_verb_route_plan(
            entry,
            canonical_lemma=self.canonical_lemma_for(entry),
            morph=self.target_form_metadata(entry),
            surface_features=self.infer_surface_features(entry),
            pos_family="v",
            exact_surface_required=True,
        )

    def _candidate_target_word_index(self, candidate: Candidate) -> int:
        if candidate.target_index < 0:
            return -1
    ...
    def _verb_route_subject(self, target: Lexeme, plan: RoutePlan) -> Tuple[str, str]:
        person_code = plan.support_constraints.get("person_code") or self.person_code_from_morph(self.target_form_metadata(target)) or "3sg"
        return PERSON_CODE_TO_SUBJECT.get(person_code, "ella"), person_code

    def _verb_route_specs(self, target: Lexeme, plan: RoutePlan, family: str) -> List[Tuple[str, List[str], int]]:
        surface = target.lemma
        canonical = self.canonical_lemma_for(target)
        subject, person_code = self._verb_route_subject(target, plan)
        specs: List[Tuple[str, List[str], int]] = []

        if family == "conditional":
            if canonical == "gustar":
                specs.extend([
                    (template_id_for_route(family, "gustar_ir"), ["me", surface, "ir"], 1),
                    (template_id_for_route(family, "gustar_comer"), ["me", surface, "comer"], 1),
                ])
            elif canonical == "deber":
                for inf in ["venir", "salir", "ir"]:
                    specs.append((template_id_for_route(family, f"deber_{inf}"), [surface, inf], 0))
            elif canonical == "haber":
                specs.extend([
                    (template_id_for_route(family, "haber_tiempo"), [surface, "tiempo"], 0),
                    (template_id_for_route(family, "haber_problemas"), [surface, "problemas"], 0),
                ])
            elif canonical == "poder":
                specs.append((template_id_for_route(family, "poder_ir"), [surface, "ir"], 0))
            return specs

        if family == "subjunctive":
            if canonical == "estar":
                specs.extend([
                    (template_id_for_route(family, "estar_aqui"), ["quiero", "que", surface, "aquí"], 2),
                    (template_id_for_route(family, "estar_bien"), ["quiero", "que", surface, "bien"], 2),
                ])
            elif canonical in {"venir", "ir", "salir", "llegar"}:
                specs.extend([
                    (template_id_for_route(family, "motion_core"), ["quiero", "que", surface], 2),
                    (template_id_for_route(family, "motion_ojala"), ["ojalá", surface], 1),
                ])
            elif canonical in {"hacer", "decir", "dar"}:
                specs.append((template_id_for_route(family, "transitive_eso"), ["quiero", "que", surface, "eso"], 2))
            else:
                specs.append((template_id_for_route(family, "generic"), ["quiero", "que", surface], 2))
            return specs

        if family == "imperative":
            if canonical in {"tomar", "beber"}:
                specs.append((template_id_for_route(family, "agua"), [surface, "agua"], 0))
            if canonical in {"escuchar", "mirar", "leer"}:
                specs.append((template_id_for_route(family, "esto"), [surface, "esto"], 0))
            if canonical == "venir":
                specs.append((template_id_for_route(family, "aqui"), [surface, "aquí"], 0))
            return specs

        if family == "finite_present":
            if canonical == "deber":
                for inf in ["mirar", "salir", "ir", "venir"]:
                    specs.append((template_id_for_route(family, f"deber_{inf}"), [surface, inf], 0))
            elif canonical == "quedar":
                specs.extend([
                    (template_id_for_route(family, "quedar_tiempo"), [surface, "tiempo"], 0),
                    (template_id_for_route(family, "quedar_claro"), [surface, "claro"], 0),
                ])
            elif canonical in {"hacer", "decir", "dar"}:
                specs.append((template_id_for_route(family, f"{canonical}_eso"), [subject, surface, "eso"], 1))
            elif canonical == "estar":
                specs.append((template_id_for_route(family, "estar_aqui"), [subject, surface, "aquí"], 1))
            else:
                specs.append((template_id_for_route(family, "generic_eso"), [subject, surface, "eso"], 1))
            return specs

        if family == "infinitive":
            if canonical in STARTER_INFINITIVE_REJECT:
                return []
            for carrier in ["quiero", "puedo", "necesito"]:
                specs.append((template_id_for_route(family, f"{carrier}_bare"), [carrier, surface], 1))
            for noun_lemma, art in STARTER_INFINITIVE_COMPLEMENTS.get(canonical, [])[:2]:
                tokens = ["quiero", surface]
                if art:
                    tokens.append(art)
                tokens.append(noun_lemma)
                specs.append((template_id_for_route(family, f"quiero_{noun_lemma}"), tokens, 1))
            return specs

        if family == "gerund":
            if canonical == "decir":
                specs.extend([
                    (template_id_for_route(family, "seguir_mismo"), ["sigo", surface, "lo", "mismo"], 1),
                    (template_id_for_route(family, "estar_eso"), ["estoy", surface, "eso"], 1),
                ])
            else:
                specs.append((template_id_for_route(family, "estar"), ["estoy", surface], 1))
            return specs

        if family == "participle":
            if canonical not in {"dar", "decir", "hacer", "leer", "comprar", "buscar", "tener"}:
                return []
            tail_map = {
                "dar": ["eso"],
                "decir": ["eso"],
            ...
            for aux in ["he", "ha"]:
                specs.append((template_id_for_route(family, f"{aux}_{canonical}"), [aux, surface] + list(tail_map.get(canonical, [])), 1))
            return specs

        return specs

    def candidate_is_structurally_safe(self, candidate: Candidate, plan: Optional[RoutePlan]) -> bool:
        return not self.verb_route_structural_reasons(candidate, plan)

    def verb_route_structural_reasons(self, candidate: Candidate, plan: Optional[RoutePlan]) -> List[str]:
        family = route_family_from_template_id(candidate.template_id)
        if not family:
            return []
        if plan is None:
            return ["missing_route_plan"]
        if family not in plan.allowed_route_families:
            return ["route_family_not_allowed"]

        words = self.word_tokens(candidate.sentence)
        if not words:
            return ["empty_route_sentence"]
        target_idx = self._candidate_target_word_index(candidate)
        target_form = normalize_token(candidate.target_form or candidate.lemma)
        if target_idx < 0 or target_idx >= len(words):
            return ["missing_route_target_index"]
        if normalize_token(words[target_idx]) != target_form or target_form != normalize_token(plan.lemma):
            return ["exact_target_surface_missing"]

        reasons: List[str] = []
        canonical = self.canonical_lemma_for(self.lexicon.get(candidate.lemma) or Lexeme(candidate.lemma, candidate.rank, candidate.pos))
        person_code = plan.support_constraints.get("person_code") or self.person_code_from_morph(self.target_form_metadata(self.lexicon.get(candidate.lemma) or Lexeme(candidate.lemma, candidate.rank, candidate.pos)))

        if family == "conditional":
            if plan.surface_class != "conditional":
                reasons.append("conditional_route_mismatch")
            if canonical == "gustar":
                if target_idx != 1 or words[0] not in {"me", "te", "nos"}:
                    reasons.append("gustar_carrier_required")
                if len(words) < 3 or self.surface_morph(words[2]).get("VerbForm") != "Inf":
                    reasons.append("conditional_infinitive_missing")
            elif canonical == "deber":
                if target_idx != 0 or len(words) < 2 or self.surface_morph(words[1]).get("VerbForm") != "Inf":
                    reasons.append("deber_conditional_infinitive")
            elif canonical == "haber":
                if target_idx != 0 or len(words) < 2 or words[1] not in {"tiempo", "problemas"}:
                    reasons.append("haber_conditional_frame")
            elif len(words) < 2:
                reasons.append("conditional_frame_incomplete")

        elif family == "subjunctive":
            if plan.surface_class != "subjunctive_finite":
                reasons.append("subjunctive_route_mismatch")
            prefix = set(words[:target_idx])
            if not ("que" in prefix or "ojalá" in prefix or "quizá" in prefix or "quizás" in prefix):
                reasons.append("missing_subjunctive_trigger")
            if canonical == "estar" and "aquí" not in words[target_idx + 1:] and "bien" not in words[target_idx + 1:]:
                reasons.append("estar_subjunctive_support_missing")

        elif family == "imperative":
            if target_idx != 0:
                reasons.append("imperative_not_sentence_initial")
            if words[0] in SUBJECT_FEATURES:
                reasons.append("imperative_with_explicit_subject")
            if any(token in {"me", "te", "se", "nos", "os", "lo", "la", "los", "las", "le", "les"} for token in words[1:]):
                reasons.append("imperative_clitic_not_safe")
            if len(words) < 2:
                reasons.append("imperative_missing_complement")
            elif words[1] not in {"agua", "esto", "aquí"}:
                reasons.append("imperative_unsafe_complement")

        elif family == "finite_present":
            if not plan.support_constraints.get("person_code") and person_code is None:
                reasons.append("missing_person_code")
            if canonical == "deber":
                if target_idx != 0 or len(words) < 2 or self.surface_morph(words[1]).get("VerbForm") != "Inf":
                    reasons.append("deber_present_infinitive")
            elif canonical == "quedar":
                if target_idx != 0 or len(words) < 2 or words[1] not in {"tiempo", "claro"}:
                    reasons.append("quedar_frame_required")
            else:
                if person_code in {"1sg", "1pl", "3pl"}:
                    expected_subject = PERSON_CODE_TO_SUBJECT.get(person_code)
                    if not words or words[0] != expected_subject:
                        reasons.append("explicit_subject_required")
                if canonical in {"hacer", "decir", "dar"} and (len(words) < target_idx + 2 or words[target_idx + 1] != "eso"):
                    reasons.append("finite_present_safe_object_missing")

        elif family == "infinitive":
            if plan.surface_class != "infinitive":
                reasons.append("infinitive_route_mismatch")
            if target_idx <= 0 or words[target_idx - 1] not in {"quiero", "puedo", "necesito"}:
                reasons.append("missing_infinitive_carrier")

        elif family == "gerund":
            if plan.surface_class != "gerund":
                reasons.append("gerund_route_mismatch")
            if target_idx <= 0 or words[target_idx - 1] not in {"sigo", "estoy"}:
                reasons.append("missing_gerund_carrier")
            if canonical == "decir" and words[:4] not in (["sigo", target_form, "lo", "mismo"], ["estoy", target_form, "eso"]):
                reasons.append("decir_gerund_frame_required")

        elif family == "participle":
            if plan.surface_class != "participle":
                reasons.append("participle_route_mismatch")
            if target_idx != 1 or words[0] not in {"he", "ha", "hemos", "han"}:
                reasons.append("missing_perfect_auxiliary")
    ...
    def _build_route_candidate(
        ...
        candidate = self.build_candidate(target, tokens, template_id, source_method, target_index)
        if not candidate:
            return None
        if not self.candidate_is_structurally_safe(candidate, plan):
            return None
    ...
    def run_route_family(self, entry: Lexeme, plan: RoutePlan, family: str, source_method: str = "template_generated") -> List[Candidate]:
        candidates: List[Candidate] = []
        for template_id, tokens, target_index in self._verb_route_specs(entry, plan, family):
            candidate = self._build_route_candidate(entry, plan, family, template_id, tokens, target_index, source_method)
            if candidate:
                candidates.append(candidate)
        return self.dedupe_candidates(candidates)

    def verb_route_candidates(self, target: Lexeme, source_method: str = "template_generated") -> List[Candidate]:
        plan = self.build_route_plan(target)
        if not plan or not plan.allowed_route_families:
            return []
        candidates: List[Candidate] = []
        for family in plan.allowed_route_families:
            candidates.extend(self.run_route_family(target, plan, family, source_method=source_method))
        return self.dedupe_candidates(candidates)

    def verb_router_smoke(self, lemmas: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        default_lemmas = [
            "gustaría", "venga", "toma", "debes", "escucha", "hago", "debemos", "hacen",
            "esté", "dado", "queda", "deberías", "habría", "diciendo", "comer",
        ]
        rows: List[Dict[str, Any]] = []
        for lemma in (lemmas or default_lemmas):
            key = normalize_token(lemma)
            target = self.lexicon.get(key)
            if not target:
                rows.append({"lemma": key, "found": False, "reason": "not_in_lexicon"})
                continue
            plan = self.build_route_plan(target)
            candidates = self.verb_route_candidates(target)
            rows.append({
                "lemma": key,
                "found": True,
                "surface_class": plan.surface_class if plan else "",
                "semantic_subclass": plan.semantic_subclass if plan else "",
                "allowed_route_families": list(plan.allowed_route_families) if plan else [],
                "candidate_count": len(candidates),
                "sentences": [cand.sentence for cand in candidates[:5]],
                "template_ids": [cand.template_id for cand in candidates[:5]],
            })
        return rows
    ...
    def seeded_template_candidate(self, target: Lexeme) -> Optional[Candidate]:
        ...
        if target.pos == "v":
            routed = self.verb_route_candidates(target, source_method="seeded_template")
            if routed:
                return self.random.choice(routed[: min(4, len(routed))])
    ...
    def pure_template_candidate(self, target: Lexeme, starter_mode: bool = False) -> Optional[Candidate]:
        ...
        if target.pos == "v":
            ...
            if not starter_mode:
                routed = self.verb_route_candidates(target, source_method="template_generated")
                if routed:
                    return self.random.choice(routed[: min(4, len(routed))])
    ...
    def emergency_pos_template_candidate(self, target: Lexeme) -> Optional[Candidate]:
        if self.normalized_pos_family(target) == "v":
            routed = self.verb_route_candidates(target, source_method="emergency_template")
            if routed:
                return max(routed, key=lambda cand: cand.score)
    ...
    def generate_strategy_candidates(self, target: Lexeme) -> List[Candidate]:
        ...
        if self.normalized_pos_family(target) == "v":
            candidates.extend(self.verb_route_candidates(target, source_method="template_generated"))
